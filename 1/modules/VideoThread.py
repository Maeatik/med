...
class VideoThread(QThread):
    ...
    def __init__(self, source=0, recordTime=0, path=def_path, fps=30, x1=0, x2=0, y1=0, y2=0):
        super().__init__()
        self.is_run = True
                ...
                if self.frame is None:
                    ...
                    if type(self.frame) == np.ndarray:
                        if self.x1 != 0 or self.x2 != 0 or self.y1 != 0 or self.y2 != 0:
                            self.frame = self.frame[self.x1:self.x2, self.y1:self.y2]
                        ...
                        if self.source == 0:
                            ...
                            if recordIsStarted:
                                if (time.time() - start) <= self.recordTime:
                                    clr = cv2.cvtColor(self.frame, cv2.COLOR_BGR2RGB)
                                    if self.x1 != 0 or self.x2 != 0 or self.y1 != 0 or self.y2 != 0:
                                        frame_re = clr[self.x1:self.x2, self.y1:self.y2]
                                    self.out1.write(frame_re)    
                                else:
                                    self.out1.release()
    ...
    def toggle_video(self):
        ...
        if self.threadOn:
            ...
            print("fsdfds",self.name)
            if self.name == "Исходный видеопоток":
