...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # анализ
        self.menuAnalize.aboutToShow.connect(lambda: Analize.openAnalize(self)) # открытие формы для анализа
    ...
    def update_frame(self, image):
        ...
        # сохраняем оригинальный формат данных
        original_format = QImage.Format.Format_RGB888 if len(image.shape) == 3 else QImage.Format.Format_Grayscale8
        ...
        if self.convertToGrayCheckBox.isChecked() or self.binarizationCheckBox.isChecked():
            ...
            image = QImage(image.data, width, height, width, original_format)
        else:
            height, width, channel = image.shape
            bytesPerLine = 3 * width
            image = QImage(image.data, width, height, bytesPerLine, original_format)
    ...
    @pyqtSlot(np.ndarray)
    def update_frameV(self, image):
        cap = cv2.VideoCapture(self.V) 
